...
extension Track {
    static func fromMessage(_ m: [String: Any], chatId: Int64, chatTitle: String) -> Track? {
        ...
        case "messageVoiceNote":
            guard let v = content["voice_note"] as? [String: Any],
                  let f = v["voice"] as? [String: Any] else { return nil }
            let dur = v["duration"] as? Int ?? 0
            return Track(chatId: chatId, messageId: m["id"] as? Int64 ?? 0,
                         chatTitle: chatTitle,
                         title: "ویس — " + Track.timeString(dur),
                         performer: "پیام صوتی",
                         duration: dur, date: date,
                         fileId: f["id"] as? Int ?? 0,
                         expectedSize: f["expected_size"] as? Int64 ?? 0,
                         thumbFileId: 0, chatPhotoFileId: 0, isVoice: true)
