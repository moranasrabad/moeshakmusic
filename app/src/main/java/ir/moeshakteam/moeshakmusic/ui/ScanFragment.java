    ...
    public void onViewCreated(@NonNull View v, @Nullable Bundle savedInstanceState) {
        ...
        btnScan.setOnClickListener(x -> {
            if (running) {
                ...
                state.setText(R.string.scan_canceling_now);
                btnScan.setText(R.string.scan_canceling);
                Ui.toast(requireContext(), R.string.scan_cancel_hint);
                return;
            }
            startScan();
        });
    ...
    private void startScan() {
        ...
        tg.scanRange(0, depth, new Tg.ScanListener() {
            ...
            public void onDone(int total) {
                main.post(() -> {
                    if (!isAdded()) return;
                    boolean wasCancel = Tg.get(requireContext()).scanCancel;
                    running = false;
                    btnScan.setText(R.string.scan_start);
                    state.setText(wasCancel ? R.string.scan_cancelled_done : R.string.scan_ready);
                    long sec = (System.currentTimeMillis() - startedAt) / 1000;
