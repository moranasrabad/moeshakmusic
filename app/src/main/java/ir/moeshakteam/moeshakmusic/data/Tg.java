...
public final class Tg implements TdClient.UpdateHandler {
    ...
    private void handleAuth(TdApi.AuthorizationState s) {
        ...
        if (s instanceof TdApi.AuthorizationStateWaitTdlibParameters) {
            ...
            chatTitles.clear();
            downloads.clear();
            // 📚 کتابخانه از دیسک دوباره لود شود — نه خالی بماند
            List<Track> libReload = LibraryPersistence.load(ctx);
            if (!libReload.isEmpty()) {
                library.addAll(libReload);
                log("📚 کتابخانه از دیسک دوباره لود شد: " + libReload.size());
            }
            TdClient.recreate();
