...
public class NeonVisualizer extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // ۱) هالهٔ مرکزی نئونی — 💥 فیکس: رنگ و position هم‌طول (۴/۴) + clamp alpha
        int glowA = Math.max(0, Math.min(255, (int) (30 + avg * 95)));
        int midA = glowA / 2;
        int[] glowColors = {0x00000000,
                (glowA << 24) | (0x22D3EE & 0x00FFFFFF),
                (midA << 24) | (0x8B5CF6 & 0x00FFFFFF),
                0x00000000};
        float[] glowPos = {0.42f, 0.72f, 0.9f, 1f};
        try {
            centerPaint.setShader(new RadialGradient(cx, cy, baseR * 1.55f,
                    glowColors, glowPos, Shader.TileMode.CLAMP));
            canvas.drawCircle(cx, cy, baseR * 1.55f, centerPaint);
        } catch (Throwable t) {
            // هرگز به‌خاطر هاله نپر — فقط رد شو
